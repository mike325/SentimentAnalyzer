...
status_msg "Setting up database"
if [[ $DB == 'postgresql' ]]; then
    if is_osx; then
        status_msg "Setting Mac pre-requirements"
        rm -rf /usr/local/var/postgres || exit 1
        initdb /usr/local/var/postgres || exit 1
        pg_ctl -wD /usr/local/var/postgres start || exit 1
        createuser -s postgres || exit 1
    fi

    postgres_drop='DROP DATABASE IF EXISTS test_ci;'
    postgres_create='create database test_ci;'

    status_msg "Postgres version: $(psql --version)"
    psql -c "$postgres_drop" -U postgres || exit 1
    psql -c "$postgres_create" -U postgres || exit 1
elif [[ $DB == 'mysql' ]]; then

    if is_osx; then
        status_msg "Setting Mac pre-requirements"
        mysql.server start || exit 1
    fi

    mysql_drop='DROP DATABASE IF EXISTS test_ci;'
    mysql_create='create database test_ci CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;'

    status_msg "MySQL version: $(mysql --version)"

    if [[ -n "$GITHUB" ]] && ! is_osx && ! is_windows; then
        mysql -uroot -proot -h127.0.0.1 --port 3306 -e "$mysql_drop" || exit 1
        mysql -uroot -proot -h127.0.0.1 --port 3306 -e "$mysql_create" || exit 1
    else
        mysql -uroot -e "$mysql_drop"   || exit 1
        mysql -uroot -e "$mysql_create" || exit 1
    fi
fi
